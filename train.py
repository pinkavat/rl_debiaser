...
def run(dataset, target, spec, log_dir = 'logs/'):
    """
        ...
        Temp hyperparameters:
            ...
            steps -- number of data item batches generated by the agent per episode, default 128
            substeps -- number of data items generated per data batch by the agent, default 32
    ...
    for episode in range(spec.get('episodes', DEFAULT_EPISODES)): # "episode" for RL, not "epoch"
        ...
        # Initial state setup
        substeps = spec.get('substeps', 32)
        prev_metric = target.get_max_equalized_odds_violation()
        state = torch.cat((torch.full((substeps, 1), prev_metric), torch.full((substeps, 1), 0.0) ), axis=1)
        ...
        for step in range(steps):
            ...
            prev_metric = metric
            next_state = torch.cat((torch.full((substeps, 1), metric), torch.full((substeps, 1), delta_metric) ), axis=1)
