...
def run(dataset, target, spec, log_dir = 'logs/'):
    """
        ...
        Temp hyperparameters:
            ...
            step_schedule -- array of steps per episode; overrides 'steps' as long as it lasts (episodes out of range revert to 'steps')
            ...
            actor_optimizer_params:
                {lr default 1e-4, etc. passed direct}
    ...
    actor_layer_spec = get_perceptron_spec(spec.get('actor_core_spec', {'hidden_layers':[400,300]}), TODO_STATE_SIZE, dataset.data_item_size)
    actor = Perceptron(actor_layer_spec)

    critic_layer_spec = get_perceptron_spec(spec.get('critic_core_spec', {'hidden_layers':[400,300]}), TODO_STATE_SIZE + dataset.data_item_size, 1) # TODO: past state add here?
    ...
    for episode in range(spec.get('episodes', DEFAULT_EPISODES)): # "episode" for RL, not "epoch"
        ...
        steps = step_schedule[episode] if episode < len(step_schedule) else spec.get('steps', 128)
        ...
        for step in range(steps):
            ...
            # TODO: temporary stopgap benchmark replacement
            if (step % 32) == 0:
