...
class RLTarget():
    ...
    def __init__(self, dataset, device_override = None, learning_rate = 1e-5, parameter_path = 'params_target.pt'):
        ...
        class TorchLogisticRegressor(torch.nn.Module):
            """ Logistic Regression Classifier. Using torch for metric compatability. """ # TODO: therefore metric REWRITE

            def __init__(self, n_in, n_out):
                """ Initialize from input and output dimension."""
                super().__init__()
        ...
        try:
            self.model = TorchLogisticRegressor(dataset.n_x + dataset.n_z, dataset.n_y).to(self.device)
            ...
            for epoch in range(50): # TODO a non-hyperparam but ought to still vary
